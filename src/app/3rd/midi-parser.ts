export const MIDIParser = {
    ...
    Uint8: function (FileAsUint8Array) {
        ...
        if (timeDivisionByte1 >= 128) {                                             // discover Time Division mode (fps or tpf)
            MIDI2['timeDivision'] = [];
            MIDI2['timeDivision'].push(timeDivisionByte1 - 128)                     // frames per second MODE  (1st byte)
            MIDI2['timeDivision'].push(timeDivisionByte2)                           // ticks in each frame     (2nd byte)
        } else {
            MIDI2['timeDivision'] = (timeDivisionByte1 * 256) + timeDivisionByte2
        }// else... ticks per beat MODE  (2 bytes value)
        //  ** read TRACK CHUNK
        for (var t = 1; t <= MIDI2.tracks; t++) {
            MIDI2.track[t - 1] = { event: [] };                                    // create new Track entry in Array
            ...
            while (!endOfTrack) {
                ...
                MIDI2.track[t - 1].event[e - 1] = {};                                 // create new event object, in events array
                MIDI2.track[t - 1].event[e - 1].deltaTime = file.readIntVLV();        // get DELTA TIME OF MIDI event (Variable Length Value)
                ...
                if (statusByte === 0xFF) {                                         // Meta Event type
                    MIDI2.track[t - 1].event[e - 1].type = 0xFF;                        // assign metaEvent code to array
                    MIDI2.track[t - 1].event[e - 1].metaType = file.readInt(1);        // assign metaEvent subtype
                    var metaEventLength = file.readIntVLV();                    // get the metaEvent length
                    switch (MIDI2.track[t - 1].event[e - 1].metaType) {
                        ...
                        case 0x06:                                              // Marker
                            MIDI2.track[t - 1].event[e - 1].data = file.readStr(metaEventLength);
                            break;
                        case 0x21:                                                 // MIDI PORT
                        case 0x59:                                                 // Key Signature
                        case 0x51:                                                // Set Tempo
                            MIDI2.track[t - 1].event[e - 1].data = file.readInt(metaEventLength);
                            break;
                        case 0x54:                                                 // SMPTE Offset
                        case 0x58:                                                 // Time Signature
                            MIDI2.track[t - 1].event[e - 1].data = [];
                            MIDI2.track[t - 1].event[e - 1].data[0] = file.readInt(1);
                            MIDI2.track[t - 1].event[e - 1].data[1] = file.readInt(1);
                            MIDI2.track[t - 1].event[e - 1].data[2] = file.readInt(1);
                            MIDI2.track[t - 1].event[e - 1].data[3] = file.readInt(1);
                            break;
                        default:
                    ...
                    MIDI2.track[t - 1].event[e - 1].type = parseInt(statusByte[0], 16);// first byte is EVENT TYPE ID
                    MIDI2.track[t - 1].event[e - 1].channel = parseInt(statusByte[1], 16);// second byte is channel
                    switch (MIDI2.track[t - 1].event[e - 1].type) {
                        case 0xF:                                                // System Exclusive Events
                            var event_length = file.readIntVLV();
                            MIDI2.track[t - 1].event[e - 1].data = file.readInt(event_length);
                        ...
                        case 0xD:                                                // Channel Aftertouch
                            MIDI2.track[t - 1].event[e - 1].data = file.readInt(1);
                            break;
                        default:
